...
if (isset($_REQUEST['login'])){
    ...
    //Search Working
    $hospitalDetails=$con->query("select * from hospital");
    $msgN=false;
    $nameShow=false;
    $provinceshow=false;
    $provincenameshow=false;
    $typeshow=false;
    $typenameshow=false;
    $typelocationshow=false;
    $typenamelocation=false;
    ...
    if (isset($_REQUEST['search'])){
        ...
        if (($name==null) && ($location=="Select") && ($type=="Select")){
            header("location:search-hospitals.php");
            return;
        }elseif (!preg_match("/^[a-zA-Z ]*$/",$name)){
            $msgN=true;
        }elseif (($name!=null) && ($location=="Select") && ($type=="Select")){
            $hospitalDetails=$con->query("select * from hospital where hos_name like '%$name%'");
            $nameShow=true;
        }elseif (($location!="Select") && ($name==null) && ($type=="Select")){
            $hospitalDetails=$con->query("select * from hospital where hos_location like '%$location%'");
            $provinceshow=true;

        }elseif (($location!="Select") && ($name!=null) && ($type=="Select")){
            $hospitalDetails=$con->query("select * from hospital where hos_name like '%$name%' and hos_location like '%$location%'");
            $provincenameshow=true;

        }elseif (($type!="Select") && ($name==null) && ($location=="Select")){
            $hospitalDetails=$con->query("select * from hospital where hos_deptDesc like '%$type%'");
            $typeshow=true;
        }elseif (($type!="Select") && ($name!=null) && ($location=="Select")){
            $hospitalDetails=$con->query("select * from hospital where hos_name like '%$name%' and hos_deptDesc like '%$type%'");
            $typenameshow=true;
        }elseif (($type!="Select") && ($name==null) && ($location!="Select")){
            $hospitalDetails=$con->query("select * from hospital where hos_location like '%$location%' and hos_deptDesc like '%$type%'");
            $typelocationshow=true;
        }elseif (($type!="Select") && ($name!=null) && ($location!="Select")){
            $hospitalDetails=$con->query("select * from hospital where hos_name like '%$name%'and hos_location like '%$location%' and hos_deptDesc like '%$type%'");
            $typenamelocation=true;
        }
...
<head>
    <meta charset="UTF-8">
    <title>Search Hospitals - Health Guide</title>
        ...
        <div class="container-fluid">
            <div class="row align-items-center">
                <div class="col-md-8 col-12">
                    <?php
                        if ($nameShow==true){
                            echo "<h3 style='color: white'> Searching: ".$name." hospital </h3>";
                        }elseif ($provinceshow==true){
                        echo "<h3 style='color: white'> Searching: Hospitals in ".$location." </h3>";
                    }elseif ($provincenameshow==true){
                            echo "<h3 style='color: white'> Searching: ".$name." hospital in ".$location." </h3>";

                        }elseif ($typeshow==true){
                            echo "<h3 style='color: white'> Searching: ".$type." hospitals </h3>";

                        }elseif ($typenameshow==true){
                            echo "<h3 style='color: white'> Searching: ".$name." hospital of ".$type." </h3>";

                        }elseif ($typelocationshow==true){
                            echo "<h3 style='color: white'> Searching: ".$type." hospital in ".$location." </h3>";

                        }elseif ($typenamelocation==true){
                            echo "<h3 style='color: white'> Searching: ".$name." hospital of ".$type." in ".$location."</h3>";
        ...
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12 col-lg-4 col-xl-3 theiaStickySidebar">
                    ...
                    <!-- Search Filter -->
                    <form action="search-hospitals.php" method="get">
                                ...
                                <div class="doctor-widget">
                                    <div class="doc-info-left">
                                        <div class="doctor-img">
                                            <a>
                                                <img src="assets/img/services/hos.png" class="img-fluid"
                                                     alt="Doctor Image">
                                            </a>
                                        </div>
                                        <div class="doc-info-cont">
                                            <h4 class="doc-name bg-s"><a> <?php echo $row['hos_name']  ?> Hospital</a></h4>
                                           <br>
