...
if(isset($_REQUEST['view'])){
    ...
    $hospitalRecord=$con->query("select * from hospital where hos_id='$hospitalID'");
    $hospitalRecordRow=$hospitalRecord->fetch_array();
    if ($hospitalRecordRow['hos_id']==null){
...
elseif (isset($_REQUEST['deleteByID'])){
    $view=true;
    $hospitalID=$_REQUEST['hos_id'];
    $row=$con->query("select hos_id from hospital where hos_id='$hospitalID'")->fetch_array();
    if($row['hos_id']==null){
        $runDeleteQueryNotSuccess=true;
    }
    else{
        $runDeleteQuery=$con->query("delete from hospital where hos_id='$hospitalID'");
        header("location:delete-hospital.php?deleteID");
...
elseif (isset($_REQUEST['deleteByIDTab'])){
    $hos_id=$_REQUEST['deleteByIDTab'];
    $deleteQuery=$con->query("delete from hospital where hos_id='$hos_id'");

    if($deleteQuery->error){
        header("location:delete-hospital.php?error");
    }
    else{
        header("location:delete-hospital.php?deletehospitalrecord");
    }

}
elseif (isset($_REQUEST['deletehospitalrecord'])){
    $view=true;
    $runDeleteQuerySuccess=true;

}
elseif (isset($_REQUEST['delete'])){
    $hos_id=$_REQUEST['delete'];
    $deleteQuery=$con->query("delete from hospital where hos_id='$hos_id'");
    ...
    if($deleteQuery->error){
        ...
        header("location:delete-hospital.php?deleteRecord");
    }
}
...
<head>
    <meta charset="utf-8">
    <title>Delete Hospital - MOHA</title>
            ...
            <div class="card-body">
                <div class="user-tabs">
                    <ul class="nav nav-tabs nav-tabs-bottom nav-justified flex-wrap">
                        <li class="nav-item">
                            <a hreflang="" class="nav-link <?php if($view==false){ echo "active"; } ?>" href="#doc-deletion" data-toggle="tab">Delete Hospitals Records <?php if(isset($_REQUEST['deleteRecord'])){ ?> <span style="color: red;">(Record Deleted)</span> <?php } ?></a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?php if($view==true){ echo "active"; } ?> " href="#doc-deletion-by-id" data-toggle="tab"><span>Delete Hospitals Record By ID</span></a>
                        </li>
                ...
                <div class="tab-content">
                        ...
                        <div class="card card-table mb-0">
                            <div class="card-body">
                                <div class="table-responsive doctorList">
                                    <form action="delete-hospital.php" method="post">
                                                ...
                                                <tr class="doctorRow">
                                                    ...
                                                    <td class="text-right">
                                                        <div class="table-action">
                                                            <button  name="delete" value="<?= $row['hos_id'] ?>"
                                        ...
                                        <div style="margin-left: 32%;margin-top: 10px;font-size: 20px;color: red; display: <?php if (!$checkRows==0){ echo "none"; }  ?>">
                                            <span>  <i class="far fa-window-close" style="margin-right: 3px"></i> No Hospital Records Avaliable! </span>
                                        </div>
                                    </form>
                                </div>
                            </div>
                    ...
                    <div id="doc-deletion-by-id" class="tab-pane fade show doctorList <?php if ($view==true){ echo "active"; }  ?>">
                        <div class="card card-table mb-0">
                            <div class="card-body">
                                <form action="delete-hospital.php" method="post" style="position: relative; margin-top: 20px;">
                                    <div class="row form-row">

                                        <div class="col-md-6" style="position: relative;margin-left: 20px;">
                                            <div class="form-group">

                                                <input type="text"  class="form-control" placeholder="enter hospital id here.." name="hos_id" required>
                                ...
                                <!--View Button Records-->
                                <div class="table-responsive doctorList2"   style="display: <?php if($viewHospitalRecord==false){ echo "none"; }  ?>">
                                    <form action="delete-hospital.php" method="post">
                                        <table class="table table-hover table-center mb-0">
                                            ...
                                            <tr class="doctorRow">
                                                <td>
                                                    <h2 class="table-avatar">
                                                        <a href="doctor-profile.html"> <?php echo $hospitalRecordRow['hos_name'] ?> <span><?php echo $hospitalRecordRow['hos_id'];  ?></span></a>
                                                    </h2>
                                                </td>
                                                <td><?=$hospitalRecordRow['hos_numOfDept'];?></td>
                                                <td><?=$hospitalRecordRow['hos_numOfDoctors'];?></td>
                                                <td><?=$hospitalRecordRow['hos_deptDesc'];?></td>
                                                <td><?=$hospitalRecordRow['hos_location'];?></td>

                                                <td><span class="badge badge-pill bg-success-light"><?=$hospitalRecordRow['hos_status'];?></span></td>
                                                <td class="text-right">
                                                    <div class="table-action">
                                                        <button name="deleteByIDTab" value="<?= $hospitalRecordRow['hos_id']; ?>" class="btn btn-sm bg-danger-light">
                                                            <i class="far fa-trash-alt"></i> Delete
                                                        </button>
                                                    </div>
                                                </td>
                                ...
                                <div style="margin-left: 35%;color: green; display: <?php if ($runDeleteQuerySuccess==false){ echo "none";}  ?>">
                                    <span>  <i class="fas fa-check" style="margin-right: 3px"></i> Hospital Record Deleted Successfuly! </span>
                                </div>
                                <div style="margin-left: 30%;color: red; display: <?php if ($runDeleteQueryNotSuccess==false){ echo "none";}  ?>">
                                    <span>  <i class="far fa-window-close" style="margin-right: 3px"></i> Hospital Record Not Deleted (No Record Avaliable)! </span>
                                </div>
                                <div style="margin-left: 35%;color: red; display: <?php if ($viewHospitalRecordEmpty==false){ echo "none";}  ?>">
                                    <span>  <i class="far fa-window-close" style="margin-right: 3px"></i> No Hospital Record Avaliable! </span>
                                </div>
                            </div>
                        </div>
                    </div>
