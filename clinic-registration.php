...
//Register doctor
    if(isset($_POST['submit'])){
        ...
        $clinicLocation=$locblock.",".$loccity.",".$locprovince;
        ...
        }if (($chckcon==true)&&($chckN==true) && ($chckT==true) && ($chckLP==true) && ($chckLC==true) && ($chckLB==true)) {

            $insert = "insert into clinics values ('','$clinicName','$clinicType','$clinicLocation','$contact','$status') ";
            $result = mysqli_query($con, $insert);

            if (!$result) {
                die ("Not registered");
            } else {
                header("location:register-successfuly.php?doctor=&hospital=&clinic=clinic&medical=&medicine=");
            }
        }
...
<head>
    <meta charset="utf-8">
		<title>Clinic Registration - MOHA</title>
...
<!--Clinic Registration Area-->
<div class="mohaForms" id="clinicRegistration">
				
				<form action="clinic-registration.php" method="post">
				<div class="card">
					<div class="card-body">
						<h4 class="card-title">Clinic Registration</h4>
						<div class="row form-row">
							...
							<div class="col-md-6">
								<div class="form-group">
									...
									<select class="form-control select" name="type">
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Select"){echo "selected";}} ?>>Select</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Eye"){echo "selected";}} ?>>Eye</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Dentist"){echo "selected";}} ?>>Dentist</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Skin"){echo "selected";}} ?>>Skin</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Primary Care"){echo "selected";}} ?>>Primary Care</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="ENT"){echo "selected";}} ?>>ENT</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Dermatologist"){echo "selected";}} ?>>Dermatologist</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Urology"){echo "selected";}} ?>>Urology</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Neurology"){echo "selected";}} ?>>Neurology</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Orthopedic"){echo "selected";}} ?>>Orthopedic</option>
                                        <option <?php if (isset($_REQUEST['type'])){ if ($_REQUEST['type']=="Cardiologist"){echo "selected";}} ?>>Cardiologist</option>

                                    </select>
                                    <small class="ml-2 text-muted bg-danger-light msgFirstName <?php if ($msgT==false){echo 'd-none'; } ?>">Select a clinic type</small>
							...
							<div class="submit-section" style="position: relative; margin-left: 400px; margin-right: 400px;">
								<button type="submit" class="btn btn-primary submit-btn" name="submit">Register Clinic</button>
							...
							<div class="doctor-actions">
								<a href="delete-clinic.php" class="btn btn-sm bg-success-light">
									<i class="fas fa-trash-alt"></i> Delete Clinics Records
								</a>
								<a href="view-clinic-records.php" class="btn btn-sm bg-info-light">
									<i class="far fa-eye"></i> View Clinics Records
								</a>
								<a href="update-clinic-records.php" class="btn btn-sm bg-default-light">
									<i class="fas fa-edit"></i> Update Clinics Records
								</a>
								
								<a href="clinic-registration.php" class="btn btn-sm bg-danger-light">
									<i class="fas fa-times"></i> Cancel Clinics Registration
								</a>
