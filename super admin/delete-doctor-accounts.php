...
$superAdminImage=$con->query("select file_name from pictures where user_id='$super_user_id'")->fetch_array()['file_name'];
    ...
    $doctorAccounts=$con->query("select * from users where user_type='doctor'");

    if (isset($_REQUEST['delete'])){
        $username=$_REQUEST['delete'];

        mysqli_query($con,"delete from users where user_username='$username'");
        header("location:delete-doctor-accounts.php?deleted");
        return;
    }
...
<head>
    ...
    <title>Delete Doctor Account - Super Admin</title>
                ...
                <ul>
                    ...
                    <li class="submenu">
                        ...
                        <ul style="display: none;">
                            ...
                            <li><a class="active" href="delete-doctor-accounts.php">Delete Doctor Account</a></li>
                        </ul>
                    </li>
                    <li class="menu-title">
        ...
        <div class="content container-fluid">
            ...
            <div class="row">
                <div class="col-md-12 d-flex">
                    ...
                    <div class="card card-table flex-fill" style="overflow-y:scroll; overflow-x:hidden; height:800px;">
                        <div class="card-header">
                            <h4 class="card-title">Doctors List <small class="float-right btn btn-sm bg-success-light <?php if (!isset($_REQUEST['deleted'])){echo 'd-none';} ?> ">Deleted Successfully !</small> </h4>
                        </div>
                        <div class="card-body">
                            <div class="table-responsive">
                                <table class="table table-hover table-center mb-0">
                                    ...
                                    <tbody>
                                    <?php
                                    while ($row=$doctorAccounts->fetch_array()) {

                                        ?>
                                        <tr>
                                            <td>
                                                <h2 class="table-avatar">
                                                    <a href="../doctor-profile.php?doc_id=<?=$row['doc_id']?>">Dr. <?=$row['user_name']?></a>
                                                </h2>
                                            </td>
                                            <td><?=$row['user_username']?></td>
                                            <td><?=$row['user_address']?></td>
                                            <td><?=$row['user_contact']?></td>
                                            <td class="<?php if ($row['user_status']=='approved'){echo 'd-block';}else{echo 'd-none';} ?>">
                                                <span class="badge badge-pill bg-success-light inv-badge">Approved</span>
                                            </td>
                                            <td class=" <?php if ($row['user_status']=='unapproved'){echo 'd-block';}else{echo 'd-none';} ?>">
                                                <span class="badge badge-pill bg-danger-light inv-badge">Unapproved</span>
                                            </td>
                                            <form action="delete-doctor-accounts.php" method="post">
                                            <td>
                                                <button type="submit" class="btn btn-sm bg-danger-light" name="delete" value="<?=$row['user_username']?>"><i class="fe fe-trash"></i>
                                                </button>
                                            </td>
                                            </form>
                                        </tr>
