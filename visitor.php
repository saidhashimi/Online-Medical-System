...
<?php
    ...
    if (isset($_REQUEST['startTime'])) {
        ...
        if (($checkCounter>0) && (!isset($_REQUEST['nocheck']))){
            if ($todayDate==$date){
                if ($nowTime>$endTime){
                    header("location:visitor.php?startTime=$startTime&endTime=$endTime&date=$date&nocheck");
                    return;
                }else{
                    header("location:appointmentBooking.php?doc_id=$doc_id$&booked");
                    return;
                }
            }else{
                header("location:appointmentBooking.php?doc_id=$doc_user_id&booked");
                return;
            }
        }
...
<?php
    if (isset($_SESSION['username'])){
        ...
        $doc_user_id=$con->query("select user_id from users where doc_id='$doc_user'")->fetch_array()['user_id'];

        if ($user_id==$doc_user_id){
            header("location:appointmentBooking.php?doc_id=$doc_user&false");
            return;
        }else {
                        ...
                        <div class="card-body">
                            ...
                            <div class="booking-doc-info">
                                ...
                                <div class="booking-info">
                                    <h4><a href="doctor-profile.php?doc_id=<?=$doc_id?>">Dr. <?php echo $doctorDetails['doc_firstName']." ".$doctorDetails['doc_lastName'];  ?></a></h4>

                                    <div class="clinic-details">
                                        <p class="doc-location"><i class="fas fa-map-marker-alt" style="margin-right: 5px"></i> <?php echo $doctorProfile['profile_city'].', '.$doctorProfile['profile_province'];  ?>, Afghanistan</p>
                                    </div>
                                </div>
                            </div>
                            <!-- Booking Doctor Info -->
                            ...
                            <div class="booking-summary">
                                <div class="booking-item-wrap">
                                    ...
                                    <ul class="booking-fee">
                                        <li>Fees <span><?=$doctorProfile['profile_fees']?>-Afg</span></li>
