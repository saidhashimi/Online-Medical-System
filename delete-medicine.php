...
if(isset($_REQUEST['view'])){
    ...
    $medicineRecord=$con->query("select * from medicineindustry where medicine_id='$medicineID'");
    $medicineRecordRow=$medicineRecord->fetch_array();
    if ($medicineRecordRow['medicine_id']==null){
...
elseif (isset($_REQUEST['deleteByID'])){
    $view=true;
    $medicineID=$_REQUEST['med_id'];
    $row=$con->query("select medicine_id from medicineindustry where medicine_id='$medicineID'")->fetch_array();
    if($row['medicine_id']==null){
        $runDeleteQueryNotSuccess=true;
    }
    else{
        $runDeleteQuery=$con->query("delete from medicineindustry where medicine_id='$medicineID'");
        header("location:delete-medicine.php?deleteID");
...
elseif (isset($_REQUEST['deleteByIDTab'])){
    ...
    $deleteQuery=$con->query("delete from medicineindustry where medicine_id='$med_id'");

    if($deleteQuery->error){
        header("location:delete-medicine.php?error");
    }
    else{
        header("location:delete-medicine.php?deletemedicinerecord");
    }

}
elseif (isset($_REQUEST['deletemedicinerecord'])){
    $view=true;
    $runDeleteQuerySuccess=true;
...
elseif (isset($_REQUEST['delete'])){
    ...
    if($deleteQuery->error){
        ...
        header("location:delete-medicine.php?deleteRecord");
    }
}
...
<head>
    <meta charset="utf-8">
    <title>Delete Medicine - MOHA</title>
...
<div class="main-wrapper">
    ...
    <!--Medicine Deletion Area-->
    <div class="mohaForms" id="medicineDeletion">

        <div class="card medicine-deletion">
            <div class="card-body">
                <div class="user-tabs">
                    <ul class="nav nav-tabs nav-tabs-bottom nav-justified flex-wrap">
                        <li class="nav-item">
                            <a class="nav-link  <?php if($view==false){ echo "active"; } ?>" href="#med-deletion" data-toggle="tab">Delete Medicine Records <?php if(isset($_REQUEST['deleteRecord'])){ ?> <span style="color: red;">(Record Deleted)</span> <?php } ?></a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link  <?php if($view==true){ echo "active"; } ?>" href="#med-deletion-by-id" data-toggle="tab"><span>Delete Medicine Record By ID</span></a>
                        </li>
                ...
                <div class="tab-content">
                                ...
                                <div class="table-responsive medicineList">
                                    <form action="delete-medicine.php" method="post">
                                                ...
                                                <tr class="medicineRow">
                                                    <td>
                                                        <h2 class="table-avatar">
                                                            <a href="doctor-profile.html"> <?php echo $row['medicine_name'] ?> <span><?php echo $row['medicine_id'] ?> </span></a>
                                                        </h2>
                                                    </td>
                                                    ...
                                                    <td><span class="badge badge-pill bg-success-light"><?php echo $row['medicine_status'] ?> </span></td>
                                                    <td class="text-right">
                                                        <div class="table-action">
                                                            <button name="delete" value="<?= $row['medicine_id'] ?>" class="btn btn-sm bg-danger-light">
                                        ...
                                        <div style="margin-left: 32%;margin-top: 10px;font-size: 20px;color: red; display: <?php if (!$checkRows==0){ echo "none"; }  ?>">
                                            <span>  <i class="far fa-window-close" style="margin-right: 3px"></i> No Medicine Records Avaliable! </span>
                                        </div>
                                    </form>
                                </div>
                            </div>
                    ...
                    <!--Medicine Deletion By ID-->
                    <div id="med-deletion-by-id" class="tab-pane fade show medicineList <?php if ($view==true){ echo "active"; }  ?>">
                        <div class="card card-table mb-0">
                            <div class="card-body">
                                <form action="delete-medicine.php" method="post" style="position: relative; margin-top: 20px;">
                                    <div class="row form-row">

                                        <div class="col-md-6" style="position: relative;margin-left: 20px;">
                                            <div class="form-group">

                                                <input type="text" name="med_id" class="form-control" placeholder="enter medicine id here.." required>
                                ...
                                <!--View Button Records-->
                                <div class="table-responsive medicineList2" id="medicineList2" style="display: <?php if($viewMedicineRecord==false){ echo "none"; }  ?>">
                                    <form action="delete-medicine.php" method="post">
                                        <table class="table table-hover table-center mb-0">
                                            ...
                                            <tr class="medicineRow">
                                                <td>
                                                    <h2 class="table-avatar">
                                                        <a href="doctor-profile.html"><?php echo $medicineRecordRow['medicine_name'] ?> <span><?php echo $medicineRecordRow['medicine_id'] ?> </span></a>
                                                    </h2>
                                                </td>
                                                ...
                                                <td><span class="badge badge-pill bg-success-light"><?php echo $medicineRecordRow['medicine_status'] ?> </span></td>
                                                <td class="text-right">
                                                    <div class="table-action">
                                                        <button name="deleteByIDTab" value="<?= $medicineRecordRow['medicine_id']; ?>" class="btn btn-sm bg-danger-light">
                                                            <i class="far fa-trash-alt"></i> Delete
                                                        </button>
                                                    </div>
                                                </td>
                                ...
                                <div style="margin-left: 35%;color: green; display: <?php if ($runDeleteQuerySuccess==false){ echo "none";}  ?>">
                                    <span>  <i class="fas fa-check" style="margin-right: 3px"></i> Medicine Record Deleted Successfuly! </span>
                                </div>
                                <div style="margin-left: 30%;color: red; display: <?php if ($runDeleteQueryNotSuccess==false){ echo "none";}  ?>">
                                    <span>  <i class="far fa-window-close" style="margin-right: 3px"></i> Medicine Record Not Deleted (No Record Avaliable)! </span>
                                </div>
                                <div style="margin-left: 35%;color: red; display: <?php if ($viewMedicineRecordEmpty==false){ echo "none";}  ?>">
                                    <span>  <i class="far fa-window-close" style="margin-right: 3px"></i> No Medicine Record Avaliable! </span>
                                </div>
