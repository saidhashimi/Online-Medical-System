...
$updateClinicRecordSuccess=false;
$updateClinicRecordNotSuccess=false;
...
if ((isset($_REQUEST['update'])) ){
    ...
    $clinicID=$_REQUEST['update'];
    $doctorRecordRow=$con->query("select * from clinics where clinic_id='$clinicID'");
...
elseif (isset($_REQUEST['viewbyid'])){
    ...
    $clinicRecordRow=$con->query("select * from clinics where clinic_id='$clinicID'");
    $chkRowsCount=mysqli_num_rows($clinicRecordRow);
...
elseif (isset($_REQUEST['viewbyname'])){
    ...
    $clinicRecordRow=$con->query("select * from clinics where clinic_name like '%$cliName%'");
    $chkRowsCount=mysqli_num_rows($clinicRecordRow);


    if (!preg_match("/^[a-zA-Z ]*$/",$cliName)){
        $msgName=true;
...
elseif (isset($_REQUEST['viewbylocation'])){
    $view=true;
    $cliLocation=$_REQUEST['cliLocation'];
    $clinicRecordRow=$con->query("select * from clinics where clinic_location like '%$cliLocation%'");
    $chkRowsCount=mysqli_num_rows($clinicRecordRow);

    if (!preg_match("/^[a-zA-Z ]*$/",$cliLocation)){
        $msgName=true;
    }elseif ($chkRowsCount==0){
        $viewClinicRecordEmpty=true;
    }
...
elseif (isset($_REQUEST['updateRecord'])){
    ...
    if ($locprovince=="Select"){
        ...
        $updateQuery = "update clinics set clinic_name='$name',clinic_type='$type',clinic_location='$location',clinic_status='Approved' where clinic_id='$id'";

        if ($con->query($updateQuery) == true) {
            header("location: update-clinic-records.php?success");
        } else {
            header("location: update-clinic-records.php?notsuccess");
...
elseif (isset($_REQUEST['updatebyView'])){
    $updateRecordArea=true;
    $view=true;
    $clinicID=$_REQUEST['updatebyView'];
    $clinicRecordRow=$con->query("select * from clinics where clinic_id='$clinicID'");
    ...
    $updateRecordValues=$clinicRecordRow->fetch_array();

}
elseif (isset($_REQUEST['success'])){
    $updateClinicRecordSuccess=true;
}
elseif(isset($_REQUEST['notsuccess'])){
    $updateClinicRecordNotSuccess=true;
}
...
<head>
    <meta charset="utf-8">
    <title>Update Clinic Records - MOHA</title>
...
<div class="main-wrapper">
    ...
    <div class="mohaForms" id="doctorUpdate">
        ...
        <div class="card doctor-update">
            <div class="card-body">
                <div class="user-tabs">
                    <div style="margin-left: 30%;color: green;font-weight: 700; display: <?php if ($updateClinicRecordSuccess==false){ echo "none";}  ?>">
                        <span>  <i class="fas fa-check" style="margin-right: 3px"></i>
                            Clinic Record Updated Successfully!!
                        </span>
                    </div>
                    <div style="margin-left: 35%;color: red;font-weight: 700; ;display: <?php if ($updateClinicRecordNotSuccess==false){ echo "none";}  ?>">
                        <span>  <i class="far fa-window-close" style="margin-right: 3px"></i>

                            Clinic Record Not Updated !!

                        </span>
                    </div>
                    <div style="margin-left: 30%;color: green;font-weight: 700; display: <?php if ($viewClinicRecord==false){ echo "none";}  ?>">


                        <span>  <i class="fas fa-check" style="margin-right: 3px"></i><?php if (isset($_REQUEST['clinicID'])){?> Clinic Record By  Search ID = <?php echo $_REQUEST['cli_id']; } ?>
                            <?php if (isset($_REQUEST['clinicName'])){?> Clinic Record Search  By  Name = <?php echo $_REQUEST['cliName']; ?>   <?php } ?>
                            <?php if (isset($_REQUEST['clinicLocation'])){?> Clinic Record   Search By Location = <?php echo $_REQUEST['cliLocation']; ?>   <?php } ?>
                        </span>
                    </div>
                    <div style="margin-left: 30%;color: red;font-weight: 700; ;display: <?php if ($viewClinicRecordEmpty==false){ echo "none";}  ?>">
                        <span>  <i class="far fa-window-close" style="margin-right: 3px"></i>

                            <?php if (isset($_REQUEST['clinicID'])){?>  No Clinic Record Avaliable By ID = <?php echo $_REQUEST['cli_id']; } ?>
                            <?php if (isset($_REQUEST['clinicName'])){?>  No Clinic Record Avaliable By Name = <?php echo $_REQUEST['cliName']; } ?>
                            <?php if (isset($_REQUEST['clinicLocation'])){?>  No Clinic Record Avaliable By Location = <?php echo $_REQUEST['cliLocation']; } ?>

                        </span>
                    </div>
                    <!--Clinic Update Record-->
                    <div class="container-fluid" style="display: <?php if ($updateRecordArea==false){ echo 'none';}  ?>">

                        <div class="row">
                            <form action="update-clinic-records.php" method="post">

                                <div class="card">
                                    <div class="card-body">
                                        <h4 class="card-title">Clinic Updation</h4>
                                        <div class="row form-row">


                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <input type="hidden" name="id" value="<?php echo $updateRecordValues['clinic_id']; if (isset($_REQUEST['updateRecord'])){echo $_REQUEST['id'];}   ?>">
                                                    <label>Clinic Name <span class="text-danger">*</span></label>

                                                    <input type="text" class="form-control" name="name" value="<?php echo $updateRecordValues['clinic_name'];   if (isset($_REQUEST['updateRecord'])){echo $_REQUEST['name'];} ?>" required>
                                                    <small class="ml-2 text-muted bg-danger-light msgFirstName <?php if ($msgC==false){echo 'd-none'; } ?>">Only letters and white space allowed</small>

                                                </div>
                                            </div>
                                            <div class="col-md-6">
                                                <div class="form-group">
                                                    <label>Clinic Type <span class="text-danger">*</span></label>

                                                    <select class="form-control select" name="type">
                                                        <option <?php if ($updateRecordValues['clinic_type']=="Eye"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="Eye"){echo "selected";}} ?>>Eye</option>
                                                        <option <?php if ($updateRecordValues['clinic_type']=="Dentist"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="Dentist"){echo "selected";}} ?>>Dentist</option>
                                                        <option <?php if ($updateRecordValues['clinic_type']=="Skin"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="Skin"){echo "selected";}} ?>>Skin</option>
                                                        <option <?php if ($updateRecordValues['clinic_type']=="Primary Care"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="Primary Care"){echo "selected";}} ?>>Primary Care</option>
                                                        <option <?php if ($updateRecordValues['clinic_type']=="ENT"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="ENT"){echo "selected";}} ?>>ENT</option>
                                                        <option <?php if ($updateRecordValues['clinic_type']=="Dermatologist"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="Dermatologist"){echo "selected";}} ?>>Dermatologist</option>
                                                        <option <?php if ($updateRecordValues['clinic_type']=="Urology"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="Urology"){echo "selected";}} ?>>Urology</option>
                                                        <option <?php if ($updateRecordValues['clinic_type']=="Neurology"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="Neurology"){echo "selected";}} ?>>Neurology</option>
                                                        <option <?php if ($updateRecordValues['clinic_type']=="Orthopedic"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="Orthopedic"){echo "selected";}} ?>>Orthopedic</option>
                                                        <option <?php if ($updateRecordValues['clinic_type']=="Cardiologist"){echo "selected";}elseif (isset($_REQUEST['updateRecord'])){if ($_REQUEST['type']=="Cardiologist"){echo "selected";}} ?>>Cardiologist</option>
                                                    </select>
                                            ...
                                            <?php
                                                $kk=$updateRecordValues['clinic_location'];
                                                $list=explode(",",$kk);
                                            ...
                                            <div class="col-md-2">
                                                <div class="form-group" style="margin-top: 30px">
                                                    <select class="form-control select" name="loccity" id="loccity">
                                                        <option <?php if (isset($_REQUEST['update'])){if ($list[1]=="Select"){echo 'selected';}} if (isset($_REQUEST['loccity'])){  if ($_REQUEST['loccity']=='Select'){echo 'selected';}} ?>>Select</option>
                                                        <?php
                                                        if ((isset($_REQUEST['update'])) || (isset($_REQUEST['loccity']))){
                                                            ?>
                                                            <option <?php if (isset($_REQUEST['update'])){echo "selected"; }elseif (isset($_REQUEST['loccity'])){  echo 'selected';} ?>><?php if (isset($_REQUEST['update'])){echo $list[1]; }elseif(isset($_REQUEST['loccity'])){ echo $_REQUEST['loccity']; }?></option>
                                            ...
                                            <div class="submit-section" style="position: relative; margin-left: 335px;">
                                                ...
                                                <a href="update-clinic-records.php" class="btn submit-btn bg-danger-light"> Cancel</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <!--View Button Records-->
                    <div class="table-responsive clinicList2Upadate"  style="display: <?php if($viewClinicRecord==false){ echo "none"; }  ?>">
                        <form action="update-clinic-records.php" method="post">
                            <table class="table table-hover table-center mb-0">
                                ...
                                <?php
                                if ($chkRowsCount==1) {
                                    $clinicRecordRow = $clinicRecordRow->fetch_array();
                                    ?>
                                    <tr class="clinicRowUpdate">
                                        <td>
                                            <h2 class="table-avatar">

                                                <a> <?php echo $clinicRecordRow['clinic_name'] ?>
                                                    <span> <?php echo $clinicRecordRow['clinic_id'] ?> </span></a>
                                            </h2>
                                        </td>
                                        ...
                                        <td>
                                            <span class="badge badge-pill bg-success-light"> <?php echo $clinicRecordRow['clinic_status'] ?></span>
                                        </td>
                                        <td class="text-right">
                                            <div class="table-action">
                                                <button  name="updatebyView" value=" <?php  echo $clinicRecordRow['clinic_id'];  ?> "  class="btn btn-sm bg-success-light">
                                ...
                                elseif ($chkRowsCount>1) {
                                    ...
                                    while ($row = $clinicRecordRow->fetch_array()) {

                                        ?>
                                        <tr class="clinicRowView">
                                            <td>
                                                <h2 class="table-avatar">
                                                    <a href="doctor-profile.html">Dr. <?php echo $row['clinic_name']; ?>
                                            ...
                                            <td class="text-right">
                                                <div class="table-action">
                                                    <button  name="updatebyView" value=" <?php  echo $row['clinic_id'];  ?> "  class="btn btn-sm bg-success-light">
                                                        <i class="fas fa-edit"></i> Update
                                                    </button>
                    ...
                    <ul class="nav nav-tabs nav-tabs-bottom nav-justified flex-wrap">
                        <li class="nav-item">
                            <a class="nav-link <?php if($view==false){ echo "active"; } ?>" href="#cli-update" data-toggle="tab" >Update Hospitla Records</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link <?php if($view==true){ echo "active"; } ?> " href="#cli-update-by" data-toggle="tab"><span>Update Clinic Record By</span></a>
                        </li>
                ...
                <div class="tab-content">
                    <!--Update clinic Record Tab-->
                    <div id="cli-update" class="tab-pane fade show <?php  if ($view==false){ echo "active"; }  ?>">
                        <div class="card card-table mb-0">
                            <div class="card-body">
                                <div class="table-responsive clinicListUpdate">
                                    <form action="update-clinic-records.php" method="get">
                                        <table class="table table-hover table-center mb-0">
                                            <thead>
                                            <tr>
                                                <th>Clinic Name</th>
                                            ...
                                            while($row=$clinicList->fetch_array()) {
                                                ...
                                                <tr class="clinicRowView">
                                                    <td>
                                                        <h2 class="table-avatar">
                                                            <a href="doctor-profile.html"> <?php echo $row['clinic_name']; ?>
                                                                <span> <?php echo $row['clinic_id'] ?> </span></a>
                                                        </h2>
                                                    </td>
                                                    <td> <?php echo $row['clinic_type'] ?></td>
                                                    ...
                                                    <td class="text-right">
                                                        <div class="table-action">
                                                            <button type="submit"  name="update" value=" <?php  echo $row['clinic_id'];  ?> "  class="btn btn-sm bg-success-light">
                                                                <i class="fas fa-edit"></i> Update
                                                            </button>
                    ...
                    <!--Clinic View By-->
                    <div id="cli-update-by" class="tab-pane fade show clinicListUpdate <?php if ($view==true){ echo "active"; }  ?>">
                        <div class="card card-table mb-0">
                            <div class="card-body">
                                <form method="post" action="update-clinic-records.php?clinicID" style="position: relative; margin-top: 20px;">
                                    <div class="row form-row">

                                        <div class="col-md-6" style="position: relative;margin-left: 20px;">
                                            <div class="form-group">

                                                <input type="number" class="form-control" placeholder="enter clinic id here.." name="cli_id" required>
                                ...
                                <!---View Clinic Record by name-->
                                <form action="update-clinic-records.php?clinicName" method="post">
                                    <div class="row form-row">

                                        <div class="col-md-6" style="position: relative;margin-left: 20px;">
                                            <div class="form-group">

                                                <input type="text" class="form-control <?php if ($msgName==true){echo 'bg-danger-light'; } ?>" value="<?php if (isset($_REQUEST['viewbyname'])){echo $_REQUEST['cliName'];} ?>" placeholder="enter clinic name here.." name="cliName" required>
                                ...
                                <form action="update-clinic-records.php?clinicLocation" method="post">
                                    <div class="row form-row">

                                        <div class="col-md-6" style="position: relative;margin-left: 20px;">
                                            <div class="form-group">

                                                <input type="text" class="form-control <?php if ($msgLocation==true){echo 'bg-danger-light'; } ?>" value="<?php if (isset($_REQUEST['viewbylocation'])){echo $_REQUEST['cliLocation'];} ?>" placeholder="enter clinic location here.." name="cliLocation" required>
